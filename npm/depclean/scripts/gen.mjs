// copied from [Oxc](https://github.com/oxc-project/oxc/blob/main/npm/depclean/scripts/generate-packages.mjs)
...
const DEPCLEAN_BIN_NAME = "depclean";
const DEPCLEAN_ROOT = resolve(fileURLToPath(import.meta.url), "../..");
...
function generateNativePackage(target) {
  const packageName = `@${DEPCLEAN_BIN_NAME}/${target}`;
  const packageRoot = resolve(PACKAGES_ROOT, `${DEPCLEAN_BIN_NAME}-${target}`);
  ...
  const depcleanBinSource = resolve(REPO_ROOT, `${DEPCLEAN_BIN_NAME}-${target}${ext}`);
  const depcleanBinTarget = resolve(packageRoot, `${DEPCLEAN_BIN_NAME}${ext}`);

  console.log(`Copy linter binary ${depcleanBinSource}`);
  fs.copyFileSync(depcleanBinSource, depcleanBinTarget);
  fs.chmodSync(depcleanBinTarget, 0o755);
}

function writeManifest() {
  const manifestPath = resolve(PACKAGES_ROOT, DEPCLEAN_BIN_NAME, "package.json");
